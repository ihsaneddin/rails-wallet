module Resourceful
  ...
  def present data, status = 200, options={}
    data = presenter(data, options)
    render json: data, status: status
  end

  def presenter data, opts= {}
    options = {}
    if data.is_a?(ActiveRecord::Relation)
      should_paginate = get_value :should_paginate
      if should_paginate
        data = paginate(data, opts[:pagination] || {})
        options[:meta] = pagination_info(data)
      end
    end
  ...
  def paginate data, options={}
    ...
    data.page(page || 1).per(per_page || 25)
  end

  def pagination_info data
    hash = {}
    if headers
      hash[:total_count] = data.total_count
      hash[:total_pages] = data.total_pages
    end
    hash
  end
